...
module OpportunitiesHelper
  ...
  def opportunity_stage_checbox(stage, count)
    ...
    check_box_tag("stage[]", stage, checked, :disabled => disabled, :onclick => remote_function(:url => { :action => :filter }, :with => %Q/"stage=" + $$("input[name='stage[]']").findAll(function (el) { return el.checked }).pluck("value")/))
  end
